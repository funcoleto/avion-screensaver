...
class GameScene: SKScene {
    ...
    func cargarPreferencias() {
        ...
        var dato = ud.float(forKey: "nube")
        cadaCuantosMinutosSaleNube = Double(dato)
    }
    
    var cadaCuantosMinutosSaleNube: Double = 1
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if(segundos >= self.cadaCuantosMinutosSaleNube){
            ...
            if(sacarNubeIzquierdaOabajo == true){//sale p izquierda COOR Y
                ...
                let coorX = CGFloat.random(in: (self.size.height * 0.01)...(self.size.height * 0.9))
